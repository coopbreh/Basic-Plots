Get_Movement_Plots <- function(name_last, name_first, date_start, date_end) {
  ...
  player_id <- playerid_lookup(last_name = name_last, first_name = name_first) %>%
    pull(mlbam_id)
  
  player_data <- statcast_search_pitchers(start_date = date_start,
                                          end_date = date_end,
                                          pitcherid = player_id)
...
Get_Movement_Plots(name_first = "Kevin", name_last = "Gausman", 
